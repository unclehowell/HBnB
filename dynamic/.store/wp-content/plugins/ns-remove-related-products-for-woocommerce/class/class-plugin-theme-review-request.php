 ...
 *
 * TODO Search & Replace rrpw_ with your prefix
 * TODO Search & Replace Rrpw_ with your prefix
 * TODO Search & Replace 'ns-remove-related-products-for-woocommerce' with your 'ns-remove-related-products-for-woocommerce'
...
class Rrpw_Modules_Reviews {
	 ...
	 * @var string
	 */
	public static $api_url = 'https://revieworgapi.nsthemes.com/wp-json/rrpw/v1/review_action';
	...
	public static function init() {
		...
		add_action( 'wp_ajax_rrpw_review_action', array( __CLASS__, 'ajax_handler' ) );
	}
	...
	public static function installed_on() {
		$installed_on = get_option( 'rrpw_reviews_installed_on', false );

		if ( ! $installed_on ) {
			$installed_on = current_time( 'mysql' );
			update_option( 'rrpw_reviews_installed_on', $installed_on );
		}
	...
	public static function ajax_handler() {
		...
		if ( ! wp_verify_nonce( $_REQUEST['nonce'], 'rrpw_review_action' ) ) {
			wp_send_json_error();
		}
		...
		try {
			...
			update_user_meta( $user_id, '_rrpw_reviews_dismissed_triggers', $dismissed_triggers );
			update_user_meta( $user_id, '_rrpw_reviews_last_dismissed', current_time( 'mysql' ) );
	...
	public static function dismissed_triggers() {
		...
		$dismissed_triggers = get_user_meta( $user_id, '_rrpw_reviews_dismissed_triggers', true );
	...
	public static function already_did( $set = false ) {
		...
		if ( $set ) {
			update_user_meta( $user_id, '_rrpw_reviews_already_did', true );

			return true;
		}

		return (bool) get_user_meta( $user_id, '_rrpw_reviews_already_did', true );
	}
	...
	public static function triggers( $group = null, $code = null ) {
		...
		if ( ! isset( $triggers ) ) {
			...
			$open_message = __( 'Hey, I noticed you recently use %d remove related product on your site – that’s awesome!. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.', 'ns-remove-related-products-for-woocommerce' );
			$time_message = __( "Hey, you've been using Remove Related Products for %s on your site - I hope that its been helpful. I would very much appreciate if you could quickly give it a 5-star rating on WordPress, just to help us spread the word.", 'ns-remove-related-products-for-woocommerce' );

			$triggers = apply_filters( 'rrpw_reviews_triggers', array(
				'time_installed' => array(
					'triggers' => array(
						'one_week'     => array(
							'message'    => sprintf( $time_message, __( '1 week', 'ns-remove-related-products-for-woocommerce' ) ),
							'conditions' => array(
								strtotime( self::installed_on() . ' +1 week' ) < time(),
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-remove-related-products-for-woocommerce/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'one_month'    => array(
							'message'    => sprintf( $time_message, __( '1 month', 'ns-remove-related-products-for-woocommerce' ) ),
							'conditions' => array(
						...
						'three_months' => array(
							'message'    => sprintf( $time_message, __( '3 months', 'ns-remove-related-products-for-woocommerce' ) ),
							'conditions' => array(
				...
				'open_count'     => array(
					'triggers' => array(
						'50_opens'  => array(
							...
							'conditions' => array(
								get_option( 'rrpw_total_open_count', 0 ) > 50,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-remove-related-products-for-woocommerce/reviews/?rate=5#rate-response',
							'pri'        => 10,
						),
						'100_opens' => array(
							...
							'conditions' => array(
								get_option( 'rrpw_total_open_count', 0 ) > 100,
						...
						'500_opens' => array(
							...
							'conditions' => array(
								get_option( 'rrpw_total_open_count', 0 ) > 500,
							),
							'link'       => 'https://wordpress.org/support/plugin/ns-remove-related-products-for-woocommerce/reviews/?rate=5#rate-response',
							'pri'        => 30,
						),
	...
	public static function admin_notices() {
			...
			(function ($) {
				...
				function dismiss(reason) {
					$.ajax({
						...
						url: ajaxurl,
						data: {
							action: 'rrpw_review_action',
							nonce: '<?php echo wp_create_nonce( 'rrpw_review_action' ); ?>',
							group: trigger.group,
							code: trigger.code,
							pri: trigger.pri,
							reason: reason
		...
		<div class="notice notice-success is-dismissible prefix-notice">
			...
			<p>
				<img class="logo" src="https://ps.w.org/ns-remove-related-products-for-woocommerce/assets/icon-128x128.png" />
				<strong>
			...
			<ul>
				<li>
					<a class="prefix-dismiss" target="_blank" href="https://wordpress.org/support/plugin/ns-remove-related-products-for-woocommerce/reviews/?rate=5#rate-response" data-reason="am_now">
						<strong><?php _e( 'Ok, you deserve it', 'ns-remove-related-products-for-woocommerce' ); ?></strong>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="maybe_later">
						<?php _e( 'Nope, maybe later', 'ns-remove-related-products-for-woocommerce' ); ?>
					</a>
				</li>
				<li>
					<a href="#" class="prefix-dismiss" data-reason="already_did">
						<?php _e( 'I already did', 'ns-remove-related-products-for-woocommerce' ); ?>
					</a>
				</li>
			</ul>
	...
	public static function last_dismissed() {
		...
		return get_user_meta( $user_id, '_rrpw_reviews_last_dismissed', true );
	}
