...
if (!function_exists ('api_pv_display_single_video_manual')) {
    function api_pv_display_single_video($video_url, $website_name) {
        ...
        $allow_full_fb = sanitize_text_field($plugin_options['api_pv_allow_full_fb']);
        $video_url = api_pv_convert_url_embed($video_url, $website_name);
        if ($video_url !== '' && $website_name == 'youtube') {
            ?>
                <iframe width="100%" height="<?php echo $video_height; ?>px" src="<?php echo esc_url($video_url); ?>" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            <?php
            ...
            if ($allow_full_fb == '1') {
                ...
                ?>
                    <div class="fb-video" data-href="<?php echo esc_url($video_url); ?>" data-width="<?php echo $video_width; ?>" data-height="<?php echo ($video_height - 10); ?>" data-show-text="false"></div>
                <?php
            }
        } elseif ($video_url !== '' && $website_name == 'vimeo') {
            ?>
            <iframe src="<?php echo esc_url($video_url); ?>" width="<?php echo $video_width; ?>" height="<?php echo $video_height; ?>" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>
            <?php
        } elseif ($video_url !== '' && $website_name == 'dailymotion') {
            ?>
                <iframe frameborder="0" width="<?php echo $video_width; ?>" height="<?php echo $video_height; ?>" src="<?php echo $video_url; ?>" allowfullscreen allow="autoplay"></iframe>
            <?php
        }
    }
}
