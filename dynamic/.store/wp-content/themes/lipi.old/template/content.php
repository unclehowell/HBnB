...
// excerpt
if( !empty( $lipi_theme_options['blog_default_charater_limit_excerpt'] ) ) {
	$excerpt_content_limit = $lipi_theme_options['blog_default_charater_limit_excerpt'];
} else {
	$excerpt_content_limit = 250;
}
// blog main content		
if( !empty( $lipi_theme_options['blog_default_charater_limit_blog_main_content'] ) ) {
	$blog_content_limit = $lipi_theme_options['blog_default_charater_limit_blog_main_content'];
} else {
	$blog_content_limit = 40;
}
...
<div id="post-<?php the_ID(); ?>" <?php post_class('blog'); ?>>
	...
	<?php
		...
		if ( is_single() )  {  
			...
			$bind_post_format_quote = get_post_meta( get_the_ID(), '__lipi_post_format_quote', true );
			if( $format == 'gallery' ) {
