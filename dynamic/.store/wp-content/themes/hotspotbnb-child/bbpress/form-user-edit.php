...
<form id="bbp-your-profile" action="<?php bbp_user_profile_edit_url( bbp_get_displayed_user_id() ); ?>" method="post" enctype="multipart/form-data">
	...
	<h2 class="entry-title"><?php _e( 'Name', 'lipi' ) ?></h2>

	<?php do_action( 'bbp_user_edit_before' ); ?>

	<fieldset class="bbp-form">
		<legend><?php _e( 'Name', 'lipi' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_name' ); ?>

		<div>
			<label for="first_name"><?php _e( 'First Name', 'lipi' ) ?></label>
		...
		<div>
			<label for="last_name"><?php _e( 'Last Name', 'lipi' ) ?></label>
		...
		<div>
			<label for="nickname"><?php _e( 'Nickname', 'lipi' ); ?></label>
		...
		<div>
			<label for="display_name"><?php _e( 'Display Name', 'lipi' ) ?></label>
	...
	<h2 class="entry-title"><?php _e( 'Contact Info', 'lipi' ) ?></h2>

	<fieldset class="bbp-form">
		<legend><?php _e( 'Contact Info', 'lipi' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_contact' ); ?>

		<div>
			<label for="url"><?php _e( 'Website', 'lipi' ) ?></label>
	...
	<h2 class="entry-title"><?php bbp_is_user_home_edit() ? _e( 'About Yourself', 'lipi' ) : _e( 'About the user', 'lipi' ); ?></h2>

	<fieldset class="bbp-form">
		<legend><?php bbp_is_user_home_edit() ? _e( 'About Yourself', 'lipi' ) : _e( 'About the user', 'lipi' ); ?></legend>

		<?php do_action( 'bbp_user_edit_before_about' ); ?>

		<div>
			<label for="description"><?php _e( 'Biographical Info', 'lipi' ); ?></label>
	...
	<h2 class="entry-title"><?php _e( 'Account', 'lipi' ) ?></h2>

	<fieldset class="bbp-form">
		<legend><?php _e( 'Account', 'lipi' ) ?></legend>

		<?php do_action( 'bbp_user_edit_before_account' ); ?>

		<div>
			<label for="user_login"><?php _e( 'Username', 'lipi' ); ?></label>
		...
		<div>
			<label for="email"><?php _e( 'Email', 'lipi' ); ?></label>
			...
			if ( !empty( $new_email ) && $new_email !== bbp_get_displayed_user_field( 'user_email', 'edit' ) ) : ?>
				...
				<span class="updated inline">
					...
					<?php printf( __( 'There is a pending email address change to <code>%1$s</code>. <a href="%2$s">Cancel</a>', 'lipi' ), $new_email['newemail'], esc_url( self_admin_url( 'user.php?dismiss=' . bbp_get_current_user_id()  . '_new_email' ) ) ); ?>
		...
		<div id="password">
			<label for="pass1"><?php _e( 'New Password', 'lipi' ); ?></label>
			<fieldset class="bbp-form password">
				...
				<span class="description"><?php _e( 'If you would like to change the password type a new one. Otherwise leave this blank.', 'lipi' ); ?></span>

				<input type="password" name="pass2" id="pass2" size="16" value="" autocomplete="off" tabindex="<?php bbp_tab_index(); ?>" />
				<span class="description"><?php _e( 'Type your new password again.', 'lipi' ); ?></span><br />

				<div id="pass-strength-result"></div>
				<span class="description indicator-hint"><?php _e( 'Your password should be at least ten characters long. Use upper and lower case letters, numbers, and symbols to make it even stronger.', 'lipi' ); ?></span>
			</fieldset>
		</div>
	...
	<?php if ( current_user_can( 'edit_users' ) && ! bbp_is_user_home_edit() ) : ?>
		...
		<h2 class="entry-title"><?php _e( 'User Role', 'lipi' ) ?></h2>

		<fieldset class="bbp-form">
			<legend><?php _e( 'User Role', 'lipi' ); ?></legend>
			...
			<?php if ( is_multisite() && is_super_admin() && current_user_can( 'manage_network_options' ) ) : ?>
				...
				<div>
					<label for="super_admin"><?php _e( 'Network Role', 'lipi' ); ?></label>
					<label>
						...
						<?php _e( 'Grant this user super admin privileges for the Network.', 'lipi' ); ?>
					</label>
				</div>
	...
	<fieldset class="submit">
		<legend><?php _e( 'Save Changes', 'lipi' ); ?></legend>
		<div>

			<?php bbp_edit_user_form_fields(); ?>

			<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_user_edit_submit" name="bbp_user_edit_submit" class="button submit user-submit"><?php bbp_is_user_home_edit() ? _e( 'Update Profile', 'lipi' ) : _e( 'Update User', 'lipi' ); ?></button>
		</div>
	</fieldset>
