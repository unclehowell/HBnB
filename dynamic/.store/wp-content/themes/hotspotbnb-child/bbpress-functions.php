...
class BBP_Default extends BBP_Theme_Compat {
	...
	public function localize_topic_script() {
		...
		if ( bbp_is_single_forum() ) {
			wp_localize_script( 'bbpress-forum', 'bbpForumJS', array(
				...
				'generic_ajax_error' => __( 'Something went wrong. Refresh your browser and try again.', 'lipi' ),
	...
	public function ajax_forum_subscription() {
		...
		if ( ! bbp_is_subscriptions_active() ) {
			bbp_ajax_response( false, __( 'Subscriptions are no longer active.', 'lipi' ), 300 );
		}

		// Bail if user is not logged in
		if ( ! is_user_logged_in() ) {
			bbp_ajax_response( false, __( 'Please login to subscribe to this forum.', 'lipi' ), 301 );
		}
		...
		if ( ! current_user_can( 'edit_user', $user_id ) ) {
			bbp_ajax_response( false, __( 'You do not have permission to do this.', 'lipi' ), 302 );
		}
		...
		if ( empty( $forum ) ) {
			bbp_ajax_response( false, __( 'The forum could not be found.', 'lipi' ), 303 );
		}

		// Bail if user did not take this action
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'toggle-subscription_' . $forum->ID ) ) {
			bbp_ajax_response( false, __( 'Are you sure you meant to do that?', 'lipi' ), 304 );
		}
		...
		if ( empty( $status ) ) {
			bbp_ajax_response( false, __( 'The request was unsuccessful. Please try again.', 'lipi' ), 305 );
		}
	...
	public function ajax_favorite() {
		...
		if ( ! bbp_is_favorites_active() ) {
			bbp_ajax_response( false, __( 'Favorites are no longer active.', 'lipi' ), 300 );
		}

		// Bail if user is not logged in
		if ( ! is_user_logged_in() ) {
			bbp_ajax_response( false, __( 'Please login to make this topic a favorite.', 'lipi' ), 301 );
		}
		...
		if ( empty( $topic ) ) {
			bbp_ajax_response( false, __( 'The topic could not be found.', 'lipi' ), 303 );
		}
	...
	public function ajax_subscription() {
		...
		// Bail if user is not logged in
		if ( ! is_user_logged_in() ) {
			bbp_ajax_response( false, __( 'Please login to subscribe to this topic.', 'lipi' ), 301 );
		}
...
function lipi__bbp_list_forums( $args = '' ) {
			...
			foreach ( $sub_forums as $sub_forum ) {
				...
				if ( !empty( $show_freshness_link ) ) {
					...
					if ( $forum_topic_count == 0 ) {
						...
						<div class='landing-forum-wrap-content'>" . lipi__get_last_poster_block( $sub_forum->ID ) . "</div>
						</div>
						";
					} else {
						$freshness_link = "			
	...
	function lipi__get_last_poster_block( $subforum_id = "" ) {
