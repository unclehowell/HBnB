jQuery(document).ready(function () {
	...
	jQuery('#frontend_editor_icon_edit').on('click', function (e) {
		...
		jQuery('#frontend_editor_button_cancel').val(LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_caption_cancel']);
	...
	jQuery('#frontend_editor_button_cancel').on('click', function (e) {
		"use strict";
		if (jQuery(this).val() == LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_caption_close'])
	...
	jQuery('#frontend_editor_button_save').on('click', function (e) {
		...
		jQuery.post(LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error == '') {
				lorem_ipsum_books_media_store_message_success('', LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_save_success']);
				jQuery('#frontend_editor_button_cancel').val(LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_caption_close']);
			} else {
				lorem_ipsum_books_media_store_message_warning(rez.error, LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_save_error']);
			}
		});
		e.preventDefault();
		return false;
	...
	jQuery('#frontend_editor_icon_delete').on('click', function (e) {
		"use strict";
		lorem_ipsum_books_media_store_message_confirm(LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_delete_post'], LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_delete_post_header'], function(btn) {
			"use strict";
			if (btn != 1) return;
			var data = {
			...
			jQuery.post(LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['ajax_url'], data, function(response) {
				...
				if (rez.error == '') {
					lorem_ipsum_books_media_store_message_success('', LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_delete_success']);
					setTimeout(function() { 
						window.location.href = LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['site_url'];
						}, 1000);
				} else {
					lorem_ipsum_books_media_store_message_warning(rez.error, LOREM_IPSUM_BOOKS_MEDIA_STORE_STORAGE['strings']['editor_delete_error']);
				}
			});
