...
if (!function_exists('lorem_ipsum_books_media_store_users_theme_setup')) {
	...
	function lorem_ipsum_books_media_store_users_theme_setup() {
		...
		if ( is_admin() ) {
			...
			add_action( 'show_user_profile',		'lorem_ipsum_books_media_store_add_fields_in_user_profile' );
			add_action( 'edit_user_profile',		'lorem_ipsum_books_media_store_add_fields_in_user_profile' );
	
			// Save / update additional fields from profile
			add_action( 'personal_options_update',	'lorem_ipsum_books_media_store_save_fields_in_user_profile' );
			add_action( 'edit_user_profile_update',	'lorem_ipsum_books_media_store_save_fields_in_user_profile' );
...
if (!function_exists('lorem_ipsum_books_media_store_add_fields_in_user_profile')) {
	function lorem_ipsum_books_media_store_add_fields_in_user_profile( $user ) { 
	?>
		<h3><?php esc_html_e('User Position', 'lorem-ipsum-books-media-store'); ?></h3>
		<table class="form-table">
			<tr>
				<th><label for="user_position"><?php esc_html_e('User position', 'lorem-ipsum-books-media-store'); ?>:</label></th>
				<td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
					<span class="description"><?php esc_html_e('Please, enter your position in the company', 'lorem-ipsum-books-media-store'); ?></span>
				</td>
			</tr>
		</table>
	
		<h3><?php esc_html_e('Social links', 'lorem-ipsum-books-media-store'); ?></h3>
		<table class="form-table">
		<?php
				...
				if (!empty($sn)) {
					...
					<tr>
						...
						<td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
							<span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'lorem-ipsum-books-media-store'), lorem_ipsum_books_media_store_strtoproper($sn)); ?></span>
						</td>
					</tr>
					<?php
				}
...
if (!function_exists('lorem_ipsum_books_media_store_save_fields_in_user_profile')) {
	function lorem_ipsum_books_media_store_save_fields_in_user_profile( $user_id ) {
