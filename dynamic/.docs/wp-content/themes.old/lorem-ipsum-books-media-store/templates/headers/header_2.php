...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_header_2_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_header_2_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_header_2_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'header_2',
			'mode'   => 'header',
			'title'  => esc_html__('Header 2', 'lorem-ipsum-books-media-store'),
			'icon'   => lorem_ipsum_books_media_store_get_file_url('templates/headers/images/2.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_header_2_output' ) ) {
	function lorem_ipsum_books_media_store_template_header_2_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_2 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			<div class="top_panel_wrap_inner top_panel_inner_style_2 top_panel_position_<?php echo esc_attr(lorem_ipsum_books_media_store_get_custom_option('top_panel_position')); ?>">
				...
				<div class="content_wrap">
					<div class="columns_wrap columns_fluid"><?php
						...
						if (!empty($contact_phone) || !empty($contact_email)) {
							?><div class="column-3_10 contact_field contact_phone">
						...
						?><div class="column-4_10 contact_logo">
						...
						if (function_exists('lorem_ipsum_books_media_store_exists_woocommerce') && lorem_ipsum_books_media_store_exists_woocommerce() && (lorem_ipsum_books_media_store_is_woocommerce_page() && lorem_ipsum_books_media_store_get_custom_option('show_cart')=='shop' || lorem_ipsum_books_media_store_get_custom_option('show_cart')=='always') && !(is_checkout() || is_cart() || defined('WOOCOMMERCE_CHECKOUT') || defined('WOOCOMMERCE_CART'))) {
							?><div class="column-3_10 contact_field contact_cart"><?php get_template_part(lorem_ipsum_books_media_store_get_file_slug('templates/headers/_parts/contact-info-cart.php')); ?></div><?php
						}
						?></div>
				</div>
			</div>
