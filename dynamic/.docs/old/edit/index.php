...
/*--------------------------------------------------
 | HotspotBnB Docs CMS
 +--------------------------------------------------
 | HotspotBnB Docs CMS - Version 0.1.2
 | Copyright (c) 2020 unclehowell
 | E-mail: hywelapbuckler@gmail.com
 | Local URL: http://localhost/docs-cms
 | Public URL: https://docs-cms.hotspotbnb.com
 | Last Changed: See changelog.txt
 +--------------------------------------------------
...
function curl_server_online_check(){
    if (function_exists('curl_init')){
        @$ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://docs-cms.hotspotbnb.com");
...
function getServerURL() {
	 // changed the second http to https to fix the preview  - at least for html ... still working on pdf preview 
    $url = (lowercase($_SERVER["HTTPS"]) == "on")?"https://":"https://";
...
if (!function_exists('mb_strtoupper')) {
    die('HotspotBnB Docs Editor<br>Error: Please enable "mbstring" php module.');
}
// +--------------------------------------------------
// | Config Class
// +--------------------------------------------------
...
class config {
    ...
    function __construct(){
        $this->data = array(
            'lang'=>'',
            'fm_root'=>'/var/www/html/docs/edit/docs',
            'timezone'=>'',
            'date_format'=>'Y/m/d H:i',
            'auth_pass'=>md5('Burgerking!!'),
            'error_reporting'=>1
        );
    }
    function save(){
...
// +--------------------------------------------------
// | BASE64 FILES
// | So that HotspotBnB Docs Editor can remain a single file script,
...
if(!function_exists('apache_request_headers')){
    function apache_request_headers(){
        ...
        foreach($SERVER as $key => $val) {
            if( preg_match($rx_https, $key) ) {
                $arh_key = preg_replace($rx_https, '', $key);
                $rx_matches = array();
...
function add_http($str){
    if (mb_strlen($str) > 0 && mb_strpos($str, 'https://') === false && mb_strpos($str, 'https://') === false) return 'https://'.$str;
    else return $str;
}
...
function demo_fix(){
    global $url_root;
    if (strpos($url_root,'docs-cms.hotspotbnb.com') !== false) {
        echo "
        <script language=\"Javascript\" type=\"text/javascript\">
            if (window.jQuery){
                setTimeout(function(){
                    $('div:has(a:has(img[alt=\"www.docs-cms.hotspotbnb.com\"]))').remove();
                },1000);
            }
        </script>";
    }
...
function html_header($header=""){
    ...
    <style>
        html {max-width:99.99vw!important;}
...
function frame2(){
    ...
    echo "<form style=\"display:inline-block;\" action=\"".$fm_path_info["basename"]."\" method=\"post\" target=\"_parent\">";
        ...
        if (count($fm_root_opts)>1)
        echo "<select name=drive onchange=\"set_fm_current_root(this.value)\" style=\"float:left; margin:1px 0 5px 0; margin-right:5px; padding:5px;\">".implode("\n",$fm_root_opts)."</select>";
        ...
        if ($auth_pass != md5(''))
        echo "&nbsp;<button type=\"button\" style=\"margin-bottom: 5px;\" class=\"btn \" onclick=\"logout()\" value=\"".et('Logout')."\"><i class=\"fa fa-file-go\"></i> ".et('Logout')."</button>";
        echo "</form>";
        echo "</td></tr>";
...
function dir_list_form() {
    ...
    function about_form(){
        openModalWindow('media/base64/index.html');
    }
    ...
    <table class=\"table\">
        ...
        <form action=\"".$fm_path_info["basename"]."\" method=\"post\" onsubmit=\"return test_action();\">
            <div class=\"float-left\">
                ...
                <button type=\"button\" class=\"btn\" onclick=\"shell_form()\" value=\"" . et('Terminal') . "\"><i class=\"fa fa-file-go\"></i> " . et('Terminal') . "</button>
                <button type=\"button\" class=\"btn\" onclick=\"about_form()\" value=\"".et('Image Encoder')."\"><i class=\"fa fa-glob\"></i> ".et('Image Encoder')."</button>
            </div>
        </form>
        </nobr>
        </td>
        ...
        if($entry_count){
            ...
            foreach ($entry_list as $ind=>$dir_entry) {
                ...
                if ($dir_entry["type"] == "dir") {
                    ...
                    if ( is_executable($fm_current_dir.$file) || (strlen($dir_entry["ext"]) && (strpos(".sh#", $dir_entry["ext"]."#" ) !== false)) ) $file_out[$file_count][] = "
...
function portscan_form(){
    ...
    <iframe id=\"portscanIframe\" name=\"portscanIframe\" src=\"\" scrolling=\"no\" frameborder=\"0\"></iframe>
    ";
    ...
    <!--
        ...
        function write_to_iframe(str){
            ...
            iframe_body.open();
            iframe_body.write('<style>body { margin:5px; background-color:#000; color:#fff; } </style><div style=\"width:100%; max-width:20vw; height:100%; font-family:Courier New; font-size:10pt; font-weight:normal; color:#aaa;\">'+iframe_text+'</div>');
            iframe_body.close();
        }
        ...
        write_to_iframe(get_boxed_text('HotspotBnB Docs Editor - Portscan<br><br><b>Hosts examples:</b><br>Single: phpfm.sf.net<br>Single: 192.168.0.1<br>Range: 192.168.0.1-254<br>Multiple: phpfm.sf.net,192.168.0.1,192.168.0.2'));
        ...
        function execute_portscan(){
            ...
            portscan_command_str += 'HotspotBnB Docs Editor - Portscan<br><br>';
...
function shell_form(){
    ...
    switch ($shell_form){
        ...
        default:
            ...
            $greetings = array();
            $greetings[] = 'HotspotBnB Docs Editor - Shell Terminal Emulator';
            $greetings[] = '';
...
function login_form(){
    ...
    if ($noscript && ($auth_pass == md5('') || $loggedon==$auth_pass)) {
        ...
        <form class=\"form-signin noScriptHidden mt-4\" name=\"login_form\" action=\"" . $fm_path_info["basename"] . "\" method=\"post\">
            ...
            <h2 class=\"form-signin-heading text-center\" style=\"float:right\">".et('.')."</h2>
            <a href=\"./readonly.php\" class=\"btn noIcon\" style=\"float:right\">".et('View Only')."</a>
...
function frameset(){
    ...
    <frameset cols=\"".$leftFrameWidth.",*\" framespacing=\"0\">
        <frameset rows=\"0,*\" framespacing=\"0\" frameborder=\"0\">
            ...
            <frame src=\"".$fm_path_info["basename"]."?frame=2\" name=frame2 border=\"0\" marginwidth=\"20vw\" marginheight=\"0\">
        </frameset>
...
function et($tag){
    ...
    $et['en']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['pt']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['es']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['de']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['fr']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['nl']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['it']['FileMan'] = 'HotspotBnB Docs Editor';
    ...
    $et['ca']['FileMan'] = 'HotspotBnB Docs Editor';
