...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_events_1_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_events_1_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_events_1_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'events-1',
			'template' => 'events-1',
			'mode'   => 'events',
			'title'  => esc_html__('Events /Style 1/', 'lorem-ipsum-books-media-store'),
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_events_1_output' ) ) {
	function lorem_ipsum_books_media_store_template_events_1_output($post_options, $post_data) {
		$show_title = true;
				...
				<div class="sc_events_item_content">
					...
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h5 class="sc_events_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></a></h5><?php
						} else {
							?><h5 class="sc_events_item_title"><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></h5><?php
						}
					}
					?>
						...
						if ($post_data['post_protected']) {
							...
							if ($post_data['post_excerpt']) {
								echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
										...
										: '<p>'.trim(lorem_ipsum_books_media_store_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																										? $post_options['descr'] 
																										: lorem_ipsum_books_media_store_get_custom_option('post_excerpt_maxlength_masonry'))).'</p>';
							}
							if (!empty($post_data['post_link']) && !lorem_ipsum_books_media_store_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_events_item_readmore"><?php lorem_ipsum_books_media_store_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a><?php
							}
						}
						?>
					</div>
