...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_news_magazine_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_news_magazine_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_news_magazine_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'lorem-ipsum-books-media-store'),
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_news_magazine_output' ) ) {
	function lorem_ipsum_books_media_store_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_html($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></a></<?php echo esc_html($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_html($title_tag); ?> class="post_title entry-title"><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
							<?php
						}
					}
			...
			if ($number <= $featured) {
					...
					if ($post_data['post_protected']) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(lorem_ipsum_books_media_store_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: lorem_ipsum_books_media_store_get_custom_option('post_excerpt_maxlength_masonry')
																)
											);
						}
					}
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//lorem_ipsum_books_media_store_get_theme_option('blog_counters');
