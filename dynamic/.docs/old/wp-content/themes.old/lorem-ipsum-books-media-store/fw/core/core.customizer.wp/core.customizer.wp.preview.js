...
( function( $ ) {
	...
	if (!lorem_ipsum_books_media_store_previewer_vars['need_refresh']) {
		var $style = $('#lorem_ipsum_books_media_store-customizer-css');
		// Prepare inline styles in the preview window
		if ( $style.length == 0 ) {
			$style = $('head').append( '<'+'style type="text/css" id="lorem_ipsum_books_media_store-customizer-css"></'+'style'+'>' ).find('#lorem_ipsum_books_media_store-customizer-css');
		}
	}
	...
	api.bind( 'preview-ready', function() {
		...
		api.preview.bind( 'refresh-preview', function( ) {
			"use strict";
			lorem_ipsum_books_media_store_set_cookie(lorem_ipsum_books_media_store_previewer_vars['options_prefix']+'_compile_less', 1);
		} );
