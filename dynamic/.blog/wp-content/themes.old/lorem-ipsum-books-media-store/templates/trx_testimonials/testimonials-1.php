...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_testimonials_1_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_testimonials_1_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_testimonials_1_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'testimonials-1',
			'template' => 'testimonials-1',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 1/', 'lorem-ipsum-books-media-store'),
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_testimonials_1_output' ) ) {
	function lorem_ipsum_books_media_store_template_testimonials_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				<div class="sc_testimonial_content"><?php lorem_ipsum_books_media_store_show_layout($post_data['post_content']); ?></div>
