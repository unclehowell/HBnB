...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_single_team_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_single_team_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_single_team_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'single-team',
			'mode'   => 'team',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Single Team member', 'lorem-ipsum-books-media-store'),
			'thumb_title'  => esc_html__('Large team image (crop)', 'lorem-ipsum-books-media-store'),
			'w'		 => 570,
			'h'		 => 390
		));
	}
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_single_team_output' ) ) {
	function lorem_ipsum_books_media_store_template_single_team_output($post_options, $post_data) {
		...
		if ($show_title && $post_options['location'] != 'center' && lorem_ipsum_books_media_store_get_custom_option('show_page_title')=='no') {
			?>
			<<?php echo esc_html($title_tag); ?> itemprop="name" class="post_title entry-title"><span class="post_icon <?php echo esc_attr($post_data['post_icon']); ?>"></span><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
			<?php 
		}
