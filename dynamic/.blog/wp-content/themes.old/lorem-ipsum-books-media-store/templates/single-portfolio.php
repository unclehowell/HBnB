...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_single_portfolio_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_single_portfolio_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_single_portfolio_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'single-portfolio',
			'mode'   => 'single',
			'need_content' => true,
			'need_terms' => true,
			'title'  => esc_html__('Portfolio item', 'lorem-ipsum-books-media-store'),
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_single_portfolio_output' ) ) {
	function lorem_ipsum_books_media_store_template_single_portfolio_output($post_options, $post_data) {
		...
		lorem_ipsum_books_media_store_template_set_args('prev-next-block', array(
			'post_options' => $post_options,
			'post_data' => $post_data
		));
		get_template_part(lorem_ipsum_books_media_store_get_file_slug('templates/_parts/prev-next-block.php'));
		...
		if ($show_title) {
			?>
			<h1 itemprop="<?php echo (float) $avg_author > 0 || (float) $avg_users > 0 ? 'itemReviewed' : 'headline'; ?>" class="post_title entry-title"><?php lorem_ipsum_books_media_store_show_layout($post_data['post_title']); ?></h1>
			<?php
		}
		...
		if ($post_data['post_protected']) {
			...
			if (lorem_ipsum_books_media_store_get_custom_option('show_post_tags')=='yes' && !empty($post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links)) {
				?>
				<div class="post_info">
					<span class="post_info_item post_info_tags"><?php esc_html_e('in', 'lorem-ipsum-books-media-store'); ?> <?php echo join(', ', $post_data['post_terms'][$post_data['post_taxonomy_tags']]->terms_links); ?></span>
				</div>
				<?php
			} 
		}
