...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_players_1_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_players_1_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_players_1_theme_setup() { 
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'players-1',
			'template' => 'players-1',
			'mode'   => 'players',
			'title'  => esc_html__('Players /Style 1/', 'lorem-ipsum-books-media-store'),
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_players_1_output' ) ) {
	function lorem_ipsum_books_media_store_template_players_1_output($post_options, $post_data) { 
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_player_avatar"><?php lorem_ipsum_books_media_store_show_layout($post_options['photo']); ?>
					<div class="sc_player_hover">
						<?php
						if (!empty($post_options['socials']) && $post_options['socials'] != 'inherit'){?>
							<div class="sc_player_socials"><?php lorem_ipsum_books_media_store_show_layout($post_options['socials']); ?></div>
						<?php } ?>
					</div>
				</div>
				<div class="sc_player_info">
					...
					if(!empty($post_options['club']) && $post_options['club'] != 'inherit'){?>
						<div class="sc_player_club">
							<?php lorem_ipsum_books_media_store_show_layout($post_options['club']);?>
						</div>
					<?php } ?>
				</div>
			</div>
