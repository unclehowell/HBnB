...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_news_excerpt_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_news_excerpt_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_news_excerpt_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'news-excerpt',
			'template' => 'news-excerpt',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Excerpt/', 'lorem-ipsum-books-media-store'),
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_news_excerpt_output' ) ) {
	function lorem_ipsum_books_media_store_template_news_excerpt_output($post_options, $post_data) {
						...
						if ($post_data['post_excerpt']) {
							echo in_array($post_data['post_format'], array('quote', 'link', 'chat', 'aside', 'status')) 
									...
									: wpautop(lorem_ipsum_books_media_store_strshort($post_data['post_excerpt'], isset($post_options['descr']) 
																								? $post_options['descr'] 
																								: lorem_ipsum_books_media_store_get_custom_option('post_excerpt_maxlength')
																)
											);
						}
					}
