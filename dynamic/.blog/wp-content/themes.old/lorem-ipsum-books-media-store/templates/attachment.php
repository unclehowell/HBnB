...
if ( !function_exists( 'lorem_ipsum_books_media_store_template_attachment_theme_setup' ) ) {
	add_action( 'lorem_ipsum_books_media_store_action_before_init_theme', 'lorem_ipsum_books_media_store_template_attachment_theme_setup', 1 );
	function lorem_ipsum_books_media_store_template_attachment_theme_setup() {
		lorem_ipsum_books_media_store_add_template(array(
			'layout' => 'attachment',
			'mode'   => 'internal',
			'title'  => esc_html__('Attachment page layout', 'lorem-ipsum-books-media-store'),
			'thumb_title'  => esc_html__('Fullwidth image', 'lorem-ipsum-books-media-store'),
			'w'		 => 1170,
			'h'		 => null,
			'h_crop' => 659
		));
...
// Template output
if ( !function_exists( 'lorem_ipsum_books_media_store_template_attachment_output' ) ) {
	function lorem_ipsum_books_media_store_template_attachment_output($post_options, $post_data) {
		...
		<article <?php post_class('post_item post_item_attachment template_attachment'); ?>>
			...
			<div class="post_featured">
				<div class="post_thumb post_nav" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					...
					if ( isset( $attachments[ $k-1 ] ) ) {
						...
						$desc = lorem_ipsum_books_media_store_strshort(!empty($attachments[ $k-1 ]->post_excerpt) ? $attachments[ $k-1 ]->post_excerpt : $attachments[ $k-1 ]->post_title, 30);
						?>
						<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Previous item', 'lorem-ipsum-books-media-store'); ?></span>
								<span class="post_nav_info_description"><?php lorem_ipsum_books_media_store_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
					}
					if ( isset( $attachments[ $k+1 ] ) ) {
						...
						<a class="post_nav_item post_nav_next" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Next item', 'lorem-ipsum-books-media-store'); ?></span>
								<span class="post_nav_info_description"><?php lorem_ipsum_books_media_store_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
			...
			<div class="post_content">
				...
				if ($post_data['post_protected']) {
					...
					echo !empty($post_data['post_content']) ? trim($post_data['post_content']) : esc_html__('No image description ...', 'lorem-ipsum-books-media-store'); 
					wp_link_pages( array( 
						'before' => '<div class="nav_pages_parts"><span class="pages">' . esc_html__( 'Pages:', 'lorem-ipsum-books-media-store' ) . '</span>', 
						'after' => '</div>',
